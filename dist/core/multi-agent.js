...
export class ModelSelector {
    ...
    selectModel(subtask) {
        ...
        if (subtask.type === 'unknown' || !this.preferences[subtask.type]) {
            return this.preferences.code;
        }
        return this.preferences[subtask.type];
    }
    /**
...
export class OrchestratorAgent {
    ...
    async executeSubtasks(plan) {
        ...
        while (completed.size < plan.subtasks.length) {
            ...
            const executable = executed.filter(st => st.status === 'pending' &&
     ...
     */
    async executeSingleSubtask(subtask) {
 ...
 */
export function createOrchestrator(providers) {
