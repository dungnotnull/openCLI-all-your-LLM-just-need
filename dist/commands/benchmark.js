 ...
 */
async function benchmarkProvider(provider, task, iterations) {
 ...
 */
async function simulateProviderCall(provider, task) {
 ...
 */
function getProviderRates(providerId) {
    const rates = {
 ...
 */
function analyzeResults(task, results, metrics) {
    ...
    for (const result of successful) {
        ...
        if (result === costWinner)
            winnerIndicators.push('💰');
        if (result === speedWinner)
            winnerIndicators.push('⚡');
        if (result === qualityWinner)
            winnerIndicators.push('⭐');
        const indicators = winnerIndicators.length ? ' ' + winnerIndicators.join(' ') : '';
        comparisonLines.push(`${result.provider}${indicators}:\n` +
 ...
 */
export function setupBenchmarkCommand(program) {
    program
        .command('benchmark')
