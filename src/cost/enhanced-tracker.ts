...
export interface EnhancedTrackerOptions {
  ...
  budgetWarningPercent?: number; // Warn at this percentage (default 0.8)
}
...
export class EnhancedCostTracker {
  ...
  private records: PeriodCostRecord[] = [];
  private storagePath: string;
  private budgetLimit?: number;
  private budgetWarningPercent: number;
   ...
   */
  private calculateCost(rates: CostRates, inputTokens: number, outputTokens: number): number {
  ...
  getProviderSummary(): ProviderCostSummary[] {
    const byProvider = new Map<string, { totalCost: number; callCount: number }>();
   ...
   */
  checkBudget(additionalCost: number = 0): { allowed: boolean; reason?: string } {
    if (!this.budgetLimit) {
      return { allowed: true };
    }
