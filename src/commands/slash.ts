  ...
  switch (cmd) {
    ...
    case '/compress':
      return await handleCompressCommand(context.session, {
        ...
        strategy: args.find(a => ['sliding', 'semantic', 'adaptive'].includes(a)) as any,
      });
...
async function handleModelSwitch(modelId: string | undefined, context: SlashContext): Promise<string> {
  if (!modelId) {
...
async function handleProviderSwitch(providerId: string | undefined, context: SlashContext): Promise<string> {
  if (!providerId) {
