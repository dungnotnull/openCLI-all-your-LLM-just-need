  ...
  .action(
    async (
      ...
      try {
        ...
        const modelDescriptor = models.find((m: ModelDescriptor) => m.id === modelId) || models[0];

        if (!modelDescriptor) {
          logger.error({ provider: providerId, model: modelId }, "Model not found");
          console.error(`\nError: Model '${modelId}' not found for provider '${providerId}'.`);
          console.error(
            `Available models: ${models.map((m: ModelDescriptor) => m.id).join(", ")}\n`
          );
          process.exit(1);
        }

        logger.info(
          { provider: provider.name, model: modelDescriptor.name },
          "Provider and model initialized"
        );
        ...
        if (task) {
          ...
          logger.info(
            { totalCost: costTracker.getSessionTotal(), callCount: costTracker.getCallCount() },
            "Session completed"
          );
